...
# binary_image = cv2.adaptiveThreshold(diff, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
...
for label in range(1, num_labels):
	...
	cv2.imshow(f'Differasdence Image{label}', isolated_object)
	# cv2.waitKey(0)
	# cv2.destroyAllWindows()
...
for i, isolated_object in enumerate(isolated_objects):
    ...
    if len(contours) > 0:
        ...
        print(f"{i}", circularity)

        if circularity > 0.8:
            # For circular objects, set orientation to 0 degrees
            orientation = 0
        else:
            # For non-circular objects, calculate the orientation as before
        ...
        image_with_line = cv2.line(
            new_image.copy(), (cx, cy), (x2, y2), (255, 0, 0), 2)
