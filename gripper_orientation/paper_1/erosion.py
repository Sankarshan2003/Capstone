...
bg_image = cv.imread('images\\img0.png')
new_image = cv.imread('images\\img1.png')


fgbg1 = cv.createBackgroundSubtractorMOG2();  

bg_gray = cv.cvtColor(bg_image, cv.COLOR_BGR2GRAY)
new_gray = cv.cvtColor(new_image, cv.COLOR_BGR2GRAY)



diff = cv.absdiff(bg_gray, new_gray)
_, binary_image = cv.threshold(diff, 80, 255, cv.THRESH_BINARY)
